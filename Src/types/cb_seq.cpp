...
namespace Cube {
    ...
    static Shared<const CB_Type> CB_Seq::get_seq_type(Shared<const CB_Type> member_type) {
    ...
    std::string CB_Seq::toS() const override {
    ...
    void CB_Seq::generate_typedef(ostream& os) const override {
    ...
    void CB_Seq::generate_literal(ostream& os, void const* raw_data, uint32_t depth = 0) const override {
        ASSERT(raw_data);
    ...
    void CB_Seq::generate_destructor(ostream& os, const std::string& id, uint32_t depth = 0) const override {
    ...
    void CB_Seq::generate_for(ostream& os, const std::string& id, const std::string& it_name = "it", uint64_t step = 1, bool reverse = false, bool protected_scope = true) const override {
    ...
    void CB_Seq::generate_for_after_scope(ostream& os, bool protected_scope = true) const override {
        if (!protected_scope) os << "}" << std::endl; // close the brace with unique iterator name
    }

    void CB_Seq::generate_index_start(ostream& os, const std::string& id) const override {
        os << id << ".v_ptr[";
    }
    void CB_Seq::generate_index_end(ostream& os) const override {
        os << "]";
    }
    ...
    static Shared<const CB_Type> CB_Fixed_Seq::get_seq_type(Shared<const CB_Type> member_type, uint32_t size) {
    ...
    std::string CB_Fixed_Seq::toS() const override {
        if (v_type == nullptr) return "_cb_unresolved_sequence";
        std::ostringstream oss;
        v_type->generate_type(oss);
        // oss << v_type->toS(); // this doesn't work for sequences of structs that contains sequences of itself
    ...
    void CB_Fixed_Seq::finalize() override {
        // set default_value
        if (size != 0) {
            ASSERT(v_type != nullptr);
    ...
    void CB_Fixed_Seq::generate_typedef(ostream& os) const override {
        ASSERT(v_type != nullptr);
        os << "typedef ";
        v_type->generate_type(os);
        os << "[";
    ...
    void CB_Fixed_Seq::generate_literal(ostream& os, void const* raw_data, uint32_t depth = 0) const override {
    ...
    void CB_Fixed_Seq::generate_destructor(ostream& os, const std::string& id, uint32_t depth = 0) const override {
        if (depth > MAX_ALLOWED_DEPTH) { post_circular_reference_error(); return; }
        CB_u32::type->generate_destructor(os, id+".size");
        CB_u32::type->generate_destructor(os, id+".capacity");
        os << "if (" << id << ".v_ptr) for(";
    ...
    void CB_Fixed_Seq::generate_for(ostream& os, const std::string& id, const std::string& it_name = "it", uint64_t step = 1, bool reverse = false, bool protected_scope = true) const override {
        uint64_t it_uid = get_unique_id();
        if(!protected_scope) os << "{ "; // open brace to put unique iterator name out of scope for the rest of the program
        // variable
        v_type->generate_type(os);
    ...
    void CB_Fixed_Seq::generate_for_after_scope(ostream& os, bool protected_scope = true) const override {
        if(!protected_scope) os << "}" << std::endl; // close the brace with unique iterator name
    }

    void CB_Fixed_Seq::generate_index_start(ostream& os, const std::string& id) const override {
        os << id << "[";
    }
    void CB_Fixed_Seq::generate_index_end(ostream& os) const override {
        os << "]";
    }
